...
void MyGLWidget::keyPressEvent(QKeyEvent* event) {
  ...
  switch (event->key()) {
    case Qt::Key_Z: {
      event->accept();
      escalat = fmin (100, escalat+2);
      update();
      break;
    }
    case Qt::Key_X: {
      event->accept();
      escalat = fmax (0, escalat-2);
      update();
      break;
    }
    default: {
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e) {
  ...
  if (DoingInteractive) {
    ...
    viewTransform ();
    emit canvi_angle_x(sx);
    emit canvi_angle_y(sy);
...
void MyGLWidget::modelTransform_p1 () {
  ...
  transform = glm::translate(transform, glm::vec3(-1, 0, -1));
  float mida = punt_Max.y-punt_Min.y;
  transform = glm::scale(transform, glm::vec3(escalat/(50*mida)));
...
void MyGLWidget::viewTransform () {
  ...
  view = glm::rotate(view, float(sx*M_PI/180), glm::vec3(1,0,0));
  view = glm::rotate(view, float(sy*M_PI/180), glm::vec3(0,1,0));
  
  glUniformMatrix4fv (viewLoc, 1, GL_FALSE, &view[0][0]);
}

void MyGLWidget::actualitzar_angle_x (int x) {
    makeCurrent();
    sx=x;
    viewTransform();
    update();
...
void MyGLWidget::actualitzar_angle_y (int y) {
    makeCurrent();
    sy=y;
    viewTransform();
    update();
